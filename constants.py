...
SUPPORTED_EXTENSIONS = {
    ".pdf": PyMuPDFLoader,
    ".docx": Docx2txtLoader,
    ".csv": "CSV_CUSTOM",  # カスタムCSVローダーの識別子
    ".txt": TextLoader
}
...
# RAG処理系
# ==========================================
# ベクターストアから取得する関連ドキュメント数（検索精度向上のため大幅増加）
RAG_RETRIEVER_K = 15
# テキストチャンクのサイズ（統合ドキュメントの完全性を保つため大幅増加）
RAG_CHUNK_SIZE = 3000
# テキストチャンクのオーバーラップ（統合ドキュメントの連続性を保つため大幅増加）
...
SYSTEM_PROMPT_INQUIRY = """
    ...
    8. 従業員情報を出力する場合、該当するすべての社員の情報を表形式で表示してください。
    9. 情報の欠落や混在を避け、各社員の完全な情報を提供してください。
    10. 社員名簿データベースから、関連する社員情報を包括的に検索・抽出してください。
    11. 部署別、スキル別、資格別、大学別の一覧情報も活用して、より多くの関連社員を見つけてください。
    12. 部分的な一致でも関連性が高い場合は積極的に回答してください。
    13. 検索結果が多すぎる場合は、関連性の高い順に整理して表示してください。
    14. 社員の氏名、部署、役職、スキル、資格、大学名などの部分的なキーワードでも該当する社員を検索してください。
